            ...
            for m in modules[i].named_modules():
                if self.freeze_bn:
                    ...
                    if isinstance(m[1], nn.Conv2d) or isinstance(m[1], SynchronizedBatchNorm2d) \
                            or isinstance(m[1], nn.BatchNorm2d) or isinstance(m[1], SEMaskBlock) or isinstance(m[1], SEMask2DBlock):
...
class MMDeepLabSEMask2DWithNorm(nn.Module):
    def __init__(self, 
        backbone='resnet', 
        output_stride=16, 
        num_classes=20,      
        ...
        super(MMDeepLabSEMask2DWithNorm, self).__init__()
        self.use_nir = use_nir
        self.use_aolp = use_aolp
        self.use_dolp = use_dolp
        self.use_pol = use_pol
        ...
        if self.use_nir:
            ...
            self.nir_aspp = build_aspp(backbone, output_stride, BatchNorm)
            self.nir_llf = SEMask2DBlock(self.low_level_feature_channels, self.low_level_feature_shape)
            self.nir_hlf = SEMask2DBlock(self.high_level_feature_channels, self.high_level_feature_shape)
            self.backbones.extend([self.nir_backbone, self.nir_llf, self.nir_hlf])
            self.decoders.append(self.nir_aspp)
        # if self.use_aolp:
        #     self.num_modalities += 1
        #     self.aolp_backbone = build_backbone(backbone, output_stride, BatchNorm, input_dim=2, pretrained=False)
        #     self.aolp_aspp = build_aspp(backbone, output_stride, BatchNorm)
        #     self.aolp_llf = SEMask2DBlock(self.low_level_feature_channels, self.low_level_feature_shape)
        #     self.aolp_hlf = SEMask2DBlock(self.high_level_feature_channels, self.high_level_feature_shape)
        #     self.backbones.extend([self.aolp_backbone, self.aolp_llf, self.aolp_hlf])
        #     self.decoders.append(self.aolp_aspp)
        # if self.use_dolp:
        #     self.num_modalities += 1
        #     self.dolp_backbone = build_backbone(backbone, output_stride, BatchNorm, input_dim=1, pretrained=False)
        #     self.dolp_aspp = build_aspp(backbone, output_stride, BatchNorm)
        #     self.dolp_llf = SEMask2DBlock(self.low_level_feature_channels, self.low_level_feature_shape)
        #     self.dolp_hlf = SEMask2DBlock(self.high_level_feature_channels, self.high_level_feature_shape)
        #     self.backbones.extend([self.dolp_backbone, self.dolp_llf, self.dolp_hlf])
        #     self.decoders.append(self.dolp_aspp)
        if self.use_pol:
            self.num_modalities += 1
            ...
            self.pol_aspp = build_aspp(backbone, output_stride, BatchNorm)
            self.pol_llf = SEMask2DBlock(self.low_level_feature_channels, self.low_level_feature_shape)
            self.pol_hlf = SEMask2DBlock(self.high_level_feature_channels, self.high_level_feature_shape)
            self.backbones.extend([self.pol_backbone, self.pol_llf, self.pol_hlf])
            self.decoders.append(self.pol_aspp)
