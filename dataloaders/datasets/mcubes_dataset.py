...
class MCubeSDataset(Dataset):
    ...
    def __getitem__(self, index):
        _img, _target, _aolp, _dolp, _nir, _nir_mask, mask, _pol = self._make_img_gt_point_pair(index)
        sample = {'image': _img, 'label': _target, 'aolp': _aolp, 'dolp': _dolp, 'nir': _nir, 'nir_mask': _nir_mask, 'u_map': self.u_map, 'v_map': self.v_map, 'mask':mask, 'pol': _pol}
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        _pol = np.stack([_aolp_sin, _aolp_cos, _dolp], axis=2)  # H x W x 3
        ...
        if self.nir_dim == 3:
            _nir = np.stack([_nir, _nir, _nir], axis=2)  # H x W x 3
        ...
        return _img[:,self._left_offset:], _target[:,self._left_offset:], \
               ...
               _nir[:,self._left_offset:], _nir_mask[:,self._left_offset:], \
               _mask[:,self._left_offset:], _pol[:,self._left_offset:]
