...
class RandomColorJitter:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            ...
            sample['brgb'] = TF.adjust_brightness(sample['brgb'], self.brightness)
            sample['bgray'] = TF.adjust_brightness(sample['bgray'], self.brightness)
            self.contrast = random.uniform(0.5, 1.5)
            sample['img'] = TF.adjust_contrast(sample['img'], self.contrast)
            sample['brgb'] = TF.adjust_contrast(sample['brgb'], self.contrast)
            sample['bgray'] = TF.adjust_contrast(sample['bgray'], self.contrast)
            self.saturation = random.uniform(0.5, 1.5)
            sample['img'] = TF.adjust_saturation(sample['img'], self.saturation)
            sample['brgb'] = TF.adjust_saturation(sample['brgb'], self.saturation)
            sample['bgray'] = TF.adjust_saturation(sample['bgray'], self.saturation)
        return sample
...
class RandomAdjustSharpness:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            ...
            sample['brgb'] = TF.adjust_sharpness(sample['brgb'], self.sharpness)
            sample['bgray'] = TF.adjust_sharpness(sample['bgray'], self.sharpness)
        return sample
...
class RandomAutoContrast:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            ...
            sample['brgb'] = TF.autocontrast(sample['brgb'])
            sample['bgray'] = TF.autocontrast(sample['bgray'])
        return sample
...
class RandomGaussianBlur:
    ...
    def __call__(self, sample: list) -> list:
        if random.random() < self.p:
            ...
            sample['brgb'] = TF.gaussian_blur(sample['brgb'], self.kernel_size)
            sample['bgray'] = TF.gaussian_blur(sample['bgray'], self.kernel_size)
...
def get_patched_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
